...
public:
  ...
  void Init() override {
    ...
    CORE_ASSERT(gladLoadGLLoader((GLADloadproc)glfwGetProcAddress),
                "Failed to initialize GLAD");

    Log(LogLevel::Info,
        std::string("OpenGL Vendor: ") + (const char *)glGetString(GL_VENDOR));
    Log(LogLevel::Info, std::string("OpenGL Renderer: ") +
                            (const char *)glGetString(GL_RENDERER));
    Log(LogLevel::Info, std::string("OpenGL Version: ") +
